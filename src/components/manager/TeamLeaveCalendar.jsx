...
import { teamLeaveCalendar } from "../../store/manager/managerAPI";
...
function TeamLeaveCalendar() {
  ...
  const loadLeaves = async () => {
    try {
      const res = await teamLeaveCalendar();
      const leaves = res.data.leaves || [];

      const formatted = leaves.map((leave) => ({
        title: `${leave.userId.userName}`,
        start: leave.from,
  ...
  return (
    <div className="card">
      ...
      <h2 className="font-bold text-lg mb-4">Team Leave Calendar</h2>
          ...
          >
            ...
            <div className="space-y-3 text-sm">
              ...
              <p>
                ...
                {selectedEvent.extendedProps.reason || "Reason not provided"}
              </p>
            </div>
